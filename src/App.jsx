...
const apiBaseUrl = "https://autoescola-api.onrender.com";
    ...
    <div className="login-screen">
      <div className="login-institucional">
        ...
        <ul className="login-features">
          ...
          <li>✓ Controle completo de Exame</li>
        </ul>
      </div>
...
function PainelInstrutor({ token, apiBaseUrl, onPix, role, onLogout }) {
  ...
  const itensFiltrados = itens.filter(item => !busca || ["nome", "cpf", "email", "telefone", "categorias", "habilitacaoNumero", "disponivel", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  ...
  const editar = (item) => {
    const f = {};
    ["nome", "cpf", "email", "telefone", "categorias", "habilitacaoNumero", "disponivel"].forEach(k => { f[k] = item[k] ?? ""; });
  ...
  return (
    <div>
      ...
      {true && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-field"><b>categorias</b><span>{item["categorias"] ?? "—"}</span></div>
            <div className="item-field"><b>habilitacaoNumero</b><span>{item["habilitacaoNumero"] ?? "—"}</span></div>
      ...
      <input type="text" value={form["categorias"] ?? ""} onChange={e => setForm({...form, categorias: e.target.value})} />
      <label className="field-label">habilitacaoNumero</label>
      <input type="text" value={form["habilitacaoNumero"] ?? ""} onChange={e => setForm({...form, habilitacaoNumero: e.target.value})} />
...
function PainelAluno({ token, apiBaseUrl, onPix, role, onLogout }) {
  ...
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["nome", "cpf", "email", "telefone", "dataNascimento", "cnh", "categoria", "totalAulas", "status", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);
  ...
  const editar = (item) => {
    const f = {};
    ["nome", "cpf", "email", "telefone", "dataNascimento", "cnh", "categoria", "totalAulas", "status"].forEach(k => { f[k] = item[k] ?? ""; });
  ...
  return (
    <div>
            ...
            <div className="kanban-coluna" key={coluna}>
              ...
              {itensFiltrados.filter(i => String(i.status ?? "Sem status") === coluna).map(item => (
                <div className="kanban-card" key={item.id}>
                  <div className="kanban-card-titulo">{String(item["nome"] ?? "Aluno")}</div>
                  <select className="kanban-select" value={item.status ?? ""} onChange={e => {
                    fetch(apiBaseUrl + "/api/alunos/" + item.id, {
      ...
      {!modoKanban && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-field"><b>totalAulas</b><span>{item["totalAulas"] ?? "—"}</span></div>
      ...
      <label className="field-label">totalAulas</label>
      <input type="number" value={form["totalAulas"] ?? ""} onChange={e => setForm({...form, totalAulas: e.target.value})} />
...
function PainelAula({ token, apiBaseUrl, onPix, role, onLogout }) {
  ...
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["alunoId", "instrutorId", "veiculo", "dataHora", "tipo", "duracao", "local", "avaliacao", "observacoes", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);
  ...
  const editar = (item) => {
    const f = {};
    ["alunoId", "instrutorId", "veiculo", "dataHora", "tipo", "duracao", "local", "avaliacao", "observacoes"].forEach(k => { f[k] = item[k] ?? ""; });
    ["aluno", "instrutor"].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };
  ...
  return (
    <div>
      ...
      {true && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-title">{String(item["veiculo"] ?? "Aula")}</div>
            ...
            <div className="item-field"><b>local</b><span>{item["local"] ?? "—"}</span></div>
      ...
      <input type="text" value={form["veiculo"] ?? ""} onChange={e => setForm({...form, veiculo: e.target.value})} />
...
function PainelExame({ token, apiBaseUrl, onPix, role, onLogout }) {
  const [itens, setItens] = React.useState([]);
  const [carregando, setCarregando] = React.useState(true);
  const [form, setForm] = React.useState({});
  const [editId, setEditId] = React.useState(null);
  ...
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["alunoId", "tipo", "data", "resultado", "pontuacao", "tentativa", "observacoes", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);

  const carregar = () => fetch(apiBaseUrl + "/api/exames", { headers: authHeaders(token) })
    .then(r => { if (r.status === 401) { onLogout && onLogout(); return []; } return r.json(); })
    .then(data => { if (Array.isArray(data)) setItens(data); })
    .catch(() => setItens([]))
    .finally(() => setCarregando(false));
  ...
  const salvar = (e) => {
    ...
    setErroSalvar("");
    setSalvando(true);
    const url = editId ? apiBaseUrl + "/api/exames/" + editId : apiBaseUrl + "/api/exames";
    fetch(url, {
      method: editId ? "PUT" : "POST",
      headers: { "Content-Type": "application/json", ...authHeaders(token) },
      body: JSON.stringify(montarCorpo()),
  ...
  const editar = (item) => {
    const f = {};
    ["alunoId", "tipo", "data", "resultado", "pontuacao", "tentativa", "observacoes"].forEach(k => { f[k] = item[k] ?? ""; });
    ["aluno"].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };

  const apagar = (id) => fetch(apiBaseUrl + "/api/exames/" + id, { method: "DELETE", headers: authHeaders(token) })
    .then(async r => {
      if (r.status === 401) { onLogout && onLogout(); return; }
      if (!r.ok) {
        const corpo = await r.json().catch(() => ({}));
  ...
  return (
    <div>
      ...
      {true && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-title">{String(item["tipo"] ?? "Exame")}</div>
            ...
            <div className="item-field"><b>resultado</b><span>{item["resultado"] ?? "—"}</span></div>
            <div className="item-field"><b>pontuacao</b><span>{item["pontuacao"] != null ? Number(item["pontuacao"]).toLocaleString("pt-BR", {style:"currency",currency:"BRL"}) : "—"}</span></div>
            <div className="item-field"><b>tentativa</b><span>{item["tentativa"] ?? "—"}</span></div>
            <div className="item-field"><b>observacoes</b><span>{item["observacoes"] ?? "—"}</span></div>
            <div className="item-field"><b>aluno</b><span>{item.alunoId ? ((alunoList.find(o => o.id === item.alunoId) || {}).nome ?? ("#" + item.alunoId)) : "—"}</span></div>
            </div>
            <div className="item-actions">
      ...
      {modalAberto && (
        <div className="modal-overlay" onClick={() => setModalAberto(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h3>{editId ? "Editar" : "Novo"} Exame</h3>
            {erroSalvar && <div className="campo-erro modal-erro">{erroSalvar}</div>}
            <form onSubmit={salvar}>
              <label className="field-label">alunoId</label>
      <input type="number" value={form["alunoId"] ?? ""} onChange={e => setForm({...form, alunoId: e.target.value})} />
      ...
      <textarea rows={3} value={form["resultado"] ?? ""} onChange={e => setForm({...form, resultado: e.target.value})} />
      <label className="field-label">pontuacao</label>
      <input type="number" step="0.01" value={form["pontuacao"] ?? ""} onChange={e => setForm({...form, pontuacao: e.target.value})} />
      <label className="field-label">tentativa</label>
      <input type="number" value={form["tentativa"] ?? ""} onChange={e => setForm({...form, tentativa: e.target.value})} />
      <label className="field-label">observacoes</label>
      <textarea rows={3} value={form["observacoes"] ?? ""} onChange={e => setForm({...form, observacoes: e.target.value})} />
      <label className="field-label">aluno</label>
      <select value={form["aluno"] ?? ""} onChange={e => setForm({...form, aluno: e.target.value})}>
  ...
  return (
    <div className="app-shell">
      <aside className="sidebar">
        <div className="sidebar-logo">Autoescola<span className="dot">.</span></div>
        ...
        <button className={"nav-btn" + (aba === "Exame" ? " active" : "")} onClick={() => setAba("Exame")}><span className="nav-ico">🗂️</span>Exame</button>
      ...
      {aba === "Exame" && <PainelExame token={token} apiBaseUrl={apiBaseUrl} onPix={abrirPix} role={role} onLogout={sair} />}
      </main>
